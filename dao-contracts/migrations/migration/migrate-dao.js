...
async function migrateDAO({
  ...
  if (migrationParams.useDaoCreator === true) {
    ...
    if (deploymentState.DAOToken === undefined) {
      let {
        receipt,
        result
      } = await sendTx(new web3.eth.Contract(
        utils.importAbi(`./${contractsDir}/${arcVersion}/DAOToken.json`).abi,
        undefined,
        opts
      ).deploy({
    ...
    if (deploymentState.Avatar === undefined) {
      ...
      avatar = result
      await logTx(receipt, `${avatar.options.address} => Avatar ${founders}`)
    ...
    for (deploymentState.foundersReputationMintedCounter; deploymentState.foundersReputationMintedCounter < founders.length; deploymentState.foundersReputationMintedCounter++) {
      setState(deploymentState, network)
    ...
    for (deploymentState.foundersTokenMintedCounter; deploymentState.foundersTokenMintedCounter < founders.length; deploymentState.foundersTokenMintedCounter++) {
      setState(deploymentState, network)
  ...
  if (migrationParams.VotingMachinesParams !== undefined && migrationParams.VotingMachinesParams.length > 0) {
    ...
    for (deploymentState.registeredGenesisProtocolParamsCount; deploymentState.registeredGenesisProtocolParamsCount < migrationParams.VotingMachinesParams.length; deploymentState.registeredGenesisProtocolParamsCount++) {
      setState(deploymentState, network)
  ...
  if (migrationParams.schemes.SchemeRegistrar) {
    ...
    for (deploymentState.SchemeRegistrarParamsCount; deploymentState.SchemeRegistrarParamsCount < migrationParams.SchemeRegistrar.length; deploymentState.SchemeRegistrarParamsCount++) {
      setState(deploymentState, network)
  ...
  if (migrationParams.schemes.ContributionReward) {
    ...
    for (deploymentState.ContributionRewardParamsCount; deploymentState.ContributionRewardParamsCount < migrationParams.ContributionReward.length; deploymentState.ContributionRewardParamsCount++) {
      setState(deploymentState, network)
  ...
  if (migrationParams.schemes.UGenericScheme && getArcVersionNumber(arcVersion) < 34) {
    ...
    for (deploymentState.UGenericSchemeParamsCount; deploymentState.UGenericSchemeParamsCount < migrationParams.UGenericScheme.length; deploymentState.UGenericSchemeParamsCount++) {
      setState(deploymentState, network)
      let parameters = [
  ...
  if (migrationParams.schemes.GlobalConstraintRegistrar) {
    ...
    for (deploymentState.GlobalConstraintRegistrarParamsCount; deploymentState.GlobalConstraintRegistrarParamsCount < migrationParams.GlobalConstraintRegistrar.length; deploymentState.GlobalConstraintRegistrarParamsCount++) {
      setState(deploymentState, network)
      let parameters = [
  ...
  if (migrationParams.schemes.UpgradeScheme) {
    ...
    for (deploymentState.UpgradeSchemeParamsCount; deploymentState.UpgradeSchemeParamsCount < migrationParams.UpgradeScheme.length; deploymentState.UpgradeSchemeParamsCount++) {
      setState(deploymentState, network)
      let parameters = [
    ...
    if (object.runFunctions !== undefined) {
      for (let i in object.runFunctions) {
        ...
        for (let j in object.runFunctions[i].params) {
          ...
          } else if (object.runFunctions[i].params[j] === 'TokenAddress') {
            functionParams.push(daoToken.options.address)
          } else {
  ...
  if (migrationParams.StandAloneContracts) {
    ...
    for (deploymentState.standAloneContractsCounter; deploymentState.standAloneContractsCounter < len; deploymentState.standAloneContractsCounter++) {
      setState(deploymentState, network)
      ...
      if (standAlone.params !== undefined) {
        for (let i in standAlone.params) {
          ...
          } else if (standAlone.params[i] === 'TokenAddress') {
      ...
      const {
        receipt,
        result: standAloneContract
      } = await sendTx(StandAloneContract.deploy({
        data: bytecode,
  ...
  if (migrationParams.CustomSchemes) {
    ...
    for (deploymentState.CustomSchemeCounter; deploymentState.CustomSchemeCounter < len; deploymentState.CustomSchemeCounter++) {
      setState(deploymentState, network)
      ...
      if (customeScheme.address === undefined) {
        ...
        let {
          receipt,
          result
        } = await sendTx(SchemeContract.deploy({
          data: bytecode,
          arguments: null
  ...
  if (deploymentState.schemesSet !== true) {
    if (migrationParams.useDaoCreator === true) {
      ...
      for (let i = deploymentState.schemesSetCounter === undefined ? 0 : deploymentState.schemesSetCounter; i < deploymentState.schemes.length; i++) {
