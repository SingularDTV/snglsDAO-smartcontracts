...
module.exports = GenesisProtocolParams
...
const stake = async function (_testSetup, _proposalId, _vote, _amount, _staker, eventName = 'Stake') {
  ...
  const transaction = await _testSetup.stakingToken.approveAndCall(
    _testSetup.genesisProtocol.address, _amount, encodeABI, {
      from: _staker
    }
  );
  var stakeLog;
...
contract('GenesisProtocol', accounts => {
  ...
  it("should log the ExecuteProposal event", async function () {
    ...
    var tx = await testSetup.genesisProtocol.vote(proposalId, 2, 0, helpers.NULL_ADDRESS, {
      from: accounts[2]
    });
  ...
  it("should log the ExecuteProposal event after time pass for preBoostedVotePeriodLimit (decision == 2 )", async function () {
    ...
    // the decisive vote is cast now and the proposal will be executed
    var tx = await testSetup.genesisProtocol.vote(proposalId, 1, 0, helpers.NULL_ADDRESS, {
      from: accounts[2]
    });
  ...
  it('Test voteWithSpecifiedAmounts - More reputation than I own, negative reputation, etc..', async function () {
    ...
    try {
      await testSetup.genesisProtocol.vote(proposalId, 1, (testSetup.reputationArray[1] + 1), helpers.NULL_ADDRESS, {
        from: accounts[1]
      });
  ...
  it("Try to send wrong proposal id to the voting/cancel functions", async () => {
    ...
    try {
      await testSetup.genesisProtocol.vote(helpers.NULL_HASH, 1, 0, helpers.NULL_ADDRESS, {
        from: accounts[0]
      });
  ...
  it("check nonce ", async () => {
    ...
    try {
      await testSetup.stakingToken.approveAndCall(
        testSetup.genesisProtocol.address, 10, encodeABI, {
          from: accounts[0]
        }
      );
  ...
  it("reputation flow ", async () => {
    ...
    assert.equal(await testSetup.genesisProtocol.shouldBoost(proposalId), false);
    await testSetup.stakingToken.transfer(staker, 500, {
      from: accounts[0]
    });
  ...
  it("quite window ", async () => {
    ...
    var tx = await testSetup.genesisProtocol.vote(proposalId, NO, 0, helpers.NULL_ADDRESS, {
      from: accounts[0]
    }); //change winning vote
  ...
  it("quite window with tie ", async () => {
    ...
    await testSetup.genesisProtocol.vote(proposalId, NO, yesVotes, helpers.NULL_ADDRESS, {
      from: accounts[2]
    }); //change winning vote by create a tie.
    await helpers.increaseTime(15); //increase time
    await testSetup.genesisProtocol.execute(proposalId);
    var proposalInfo = await testSetup.genesisProtocol.proposals(proposalId);
    assert.equal(proposalInfo[proposalStateIndex], 6); //boosted -still not execute
  ...
  it("vote on behalf ", async function () {
    ...
    const proposalId = await propose(testSetup);
    try {
      await testSetup.genesisProtocol.vote(proposalId, 1, 0, accounts[2], {
        from: accounts[0]
      });
    ...
    let voteTX = await testSetup.genesisProtocol.vote(proposalId, 1, 0, accounts[2], {
      from: voteOnBehalf
    });
  ...
  it("quite window double toggling direction", async () => {
    ...
    await testSetup.genesisProtocol.vote(proposalId, NO, 0, helpers.NULL_ADDRESS, {
      from: accounts[2]
    }); //change winning vote and execute
    assert.equal(await testSetup.genesisProtocol.orgBoostedProposalsCnt(organizationId), 0);
  ...
  it("set organization ", async () => {
    ...
    tx = await testSetup.genesisProtocol.propose(2, testSetup.genesisProtocolParams.paramsHash, helpers.NULL_ADDRESS, accounts[1], {
      from: accounts[1]
    });
    assert.equal(tx.logs.length, 1);
    assert.equal(tx.logs[0].event, "NewProposal");
